...
class Code_App:
    def _init_(self,root):
        ...
        bg_color="#074463"
        title=Label(self.root,text="Code",bd=12,relief=GROOVE,bg=bg_color,fg="white",font=("times new roman",30,"bold"),pady=2).pack()
   #=========vaiables=====
   #====cosmetics======
        self.soap=IntVar()
  ...
  #====customer details==
        ...
        cphn_lbl=Label(F1,text="Phone",bg=bg_color,fg="white",font=("times new roman",18,"bold")).grid(row=0,column=2,padx=20,pady=5)
...
#====cold drink====
        ...
        c1_lbl=Label(F4,text="Mazaa",font=("times new roman",16,"bold"),bg=bg_color,fg="lightgreen").grid(row=0,column=0,padx=10,pady=10,sticky="w")
        c1_txt=Entry(F4,width=10,textvariable=self.mazaa,font=("times new roman",16,"bold"),bd=5,relief=SUNKEN).grid(row=0,column=1,padx=10,pady=10)
        ...
        Clear_btn=Button(btn_F,text="Clear All",command=self.clear_data,bg="cadetblue",fg="white",bd=2,pady=15,width=10,font="arial 15 bold").grid(row=0,column=2,padx=5,pady=5)
    ...
    def bill_area(self):
        if self.c_name.get()=="" or self.cphn.get()=="":
            ...
            if self.spray.get()!=0:
                self.txtarea.insert(END,f"\n  Spray\t\t{self.soap.get()}\t\t{self.c_hs_p}")    
            if self.gell.get()!=0:
                self.txtarea.insert(END,f"\n  Gel\t\t{self.soap.get()}\t\t{self.c_hg_p}")
            if self.lotion.get()!=0:
                self.txtarea.insert(END,f"\n Lotion\t\t{self.soap.get()}\t\t{self.c_bl_p}")    
       #=====grocery===
    ...
    def save_bill(self):
       ...
       op=messagebox.askyesorno("Save Bill","Do you want to save the bill?")
       if op>0:
    ...
    def find_bill(self):
      ...
      for i in os.licstdir("bills/"): 
          if i.split('.')[0]==self.serach_bill.get():
              f1=open(f"bills/{i}","r")
              self.txtarea.delete('1.0',END)
              for d in f1:
    ...
    def clear_data(self):
       ...
       op=messagebox.askyesno("Clear","Do You Reallywant to clear?")
    ...
    def Exit_app(self):
       ...
       op=messagebox.askyesno("Exit","Do You Reallywant to exit")
       if op>0:
