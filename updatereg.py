...
bg = ImageTk.PhotoImage(Image.open("update1350.jpg"))
...
#Headline------------
my_canvas.create_text(570,20,text="Edit Your Account",font=("Courier",35,"italic","bold"),fill="white")
#------------------
...
def submit():
   ...
   # for n in range(16)
   c.execute("INSERT INTO cust_reg VALUES (:uname,:email,:ucntct,:udob,:uadd,:upin,:ucp)",
             {
...
def show():
   ...
   c.execute("SELECT name,dob,oid FROM cust_reg")
   records=c.fetchall()
   print(records)
...
def update():
    ...
    res=c.execute("""UPDATE cust_reg SET
    			name = :first,
    			cntct = :ct,
    			dob=:date,
    			addres=:add,
    			...
    			WHERE email= :e""",
              {
                  'first': uname_entry.get(),
                  'ct': ucntct_entry.get(),
                  'date': udob_entry.get(),
                  'add': uadd_entry.get(),
                  'pin': upincode_entry.get(),
                  'pas':ucp_entry.get(),
                  'e':uemail_entry.get()

              })
    if res:
        messagebox.showinfo("Message", "Profile Updated Successfully")
        uname_entry.delete(0, END)
        uemail_entry.delete(0, END)
        uadd_entry.delete(0, END)
        ucntct_entry.delete(0, END)
        ...
        messagebox.showerror("Error","Error Occur While Updated Please Try Again Later")
...
update_btn = Button(root, text="Update", font=("Courier", 20, "bold"), width=15, fg="white",bg="black", command=update)
update_btn_window = my_canvas.create_window(350, 550, anchor="nw", window=update_btn)
...
show_btn_window = my_canvas.create_window(350, 450, anchor="nw", window=show_btn)
